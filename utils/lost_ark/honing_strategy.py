...
def _get_enhancement_combination_list(
    ...
    to_sort = [(_rate_and_cost(honing_level, c, cache=cache), c)
               for c in counts]
...
def _apply_combination(
    ...
    enhancement_rate, _ = _rate_and_cost(
        honing_level, combination, cache=cache)
...
def _construct_graph(
    ...
    if starting_state is None:
        starting_state = _HoningState(
            rate_permyria=honing_level.base_rate_permyria)
